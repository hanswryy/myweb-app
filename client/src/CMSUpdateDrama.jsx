        ...
        if (id) {
          ...
          const fetchDramaDetails = async () => {
            try {
                ...
                setFormData({
                    title: data.title,
                    alt_title: data.alternative_title,
                    year: data.year,
                    ...
                    genres: data.genres,
                    actors: data.actors,
                    trailer_link: data.link_trailer,
                    actorsInput: '', // Clear actors input field
                });

                console.log('fetched actors:', data.actors);
                setImagePreview(data.url_photo);
        
                // Fetch actors for autocomplete
                // const actorIds = data.actors.map((actor) => actor.id);
                // console.log('Actor IDs:', actorIds);
                // const actorResponse = await fetch(`http://localhost:3001/actors?ids=${actorIds.join(',')}`);
                // const actorData = await actorResponse.json();
                // setActorSuggestions(actorData);
            } catch (error) {
                            ...
                            <div className="w-full lg:w-4/5">
                                <form onSubmit={handleSubmit}>
                                    ...
                                    <div className="mt-2 grid grid-cols-2 sm:grid-cols-4 gap-2">
                                        ...
                                        Add more genres as needed */}
                                        {console.log('formData.genres:', formData.genres)}
